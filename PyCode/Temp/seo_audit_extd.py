...
def build_overview(crawl_df, meta_df, headings_df, canon_df, sitemap_df, comp_df,  
    ...
    try:
        overview = {
            ...
            "Meta Data": flag(meta_df["title_missing"].sum() > 0 or meta_df["description_missing"].sum() > 0),
            ...
            "Schema": flag(schema_df is None or schema_df.empty or schema_df["schema_present"].sum() == 0),
...
def save_insight_report(customer_name, logger,
    ...
    def _add_section(name, func, df, *args):
        ...
        try:
            if df is None or df.empty:
                ...
                temp_df = df.head(preview_rows).copy()
                ...
                # Handle other non-serializable types and NaNs
                temp_df = temp_df.astype(object).where(pd.notnull(temp_df), None)
                
                table_data = temp_df.to_dict(orient='records')
        ...
        try:
            ...
            for col in overview_df.columns:
                ...
                if val == "ok":
                    ...
                    top_metrics[col] = "🔴 Issue"
        except Exception as e:
            logger.warning(f"Could not build overview metrics: {e}")

    # --- Final JSON structure ---
    report_data = {
        "customer_name": customer_name,
        "report_title": "AEO Audit Report",
        "key_recommendations": client_actions,
        "dominant_rendering_mode": dominant_mode,
...
def main(customer_name, url, domain_regex=None):
    ...
    log_dir = f"./output/logs/{date_str}/{customer_cap}"
    ...
    # ---------------- Build JSON Report ----------------
    final_report_data = save_insight_report(customer_name, logger,
                                            meta_df, headings_df, canon_df, status_df,
                                            comp_df, url_struct_df, redirects_df,
                                            nodes_df, edges_df,
    ...
    slug = slugify(customer_name)   # e.g. "Boat Lifestyle" → "boat-lifestyle"

    output_file = f"{output_dir}/{slug}_seo_audit_data.json"
